...
window.onload = function () {
  ...
  //Cuando pinchamos en las categorias: Accion, comedia etc. Debe redirigirlo a la pagina de la categoria pasando
  //el parametro de la categoria en la url
...
//Cogemos la URL que nos llegue y la parseamos para coger los datos que nos interesa
...
//Para llamar a la funcion de recibir los datos de la api con la busqueda que queramos
const data_search = SEARCH_URL + busqueda + "&" + API_KEY;
console.log(data_search);
getData(SEARCH_URL + busqueda + "&" + API_KEY);

//Esta funcion se encarga de mostrar los resultados mientras se escribe en el input
...
function predictTexts(data) {
  //Cuando borramos todo el contenido del input no queremos que se nos guarde la ultima busqueda asi que la limpiamos
  clearPredict();
  ...
  if (data.length > 0) {
    ...
    //En las busquedas a veces las pelis o series tienen el mismo nombre pero en distintas versiones entonces necesitamos
    //remover las duplicados
    removeDuplicates(peliculas);

    //Le aplicamos un limitador para como maximo nos salga 10 busquedas
...
//Esta funcion es la llamada a la api donde le pasamos la url de la busqueda y debemos recoger los datos
//Y parsearlos a json para manipularlos
...
//Esta funcion es la que muestra los datos en el html con el objeto json ya parseado
function showData(data) {
  ...
  data.forEach((movie) => {
    ...
    //Creamos un contador para limitar las busquedas a 6
    if (count <= 6) {
      movieEl.innerHTML = `
      ...
      //Si pinchamos en cualquier pelicula nos debe redirigir a la pelicula en cuestion donde pasamos el id de la peli
  ...
  //Si pinchamos en el texto de la busqueda nos debe redirigir a la pagina de busqueda con la busqueda que hemos escrito
