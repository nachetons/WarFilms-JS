...
const API_URL_CATEGORY =
  URL_BASE +
  "discover/movie?sort_by=popularity.desc&vote_count.gte=2000&" +
  API_KEY +
  "&with_genres=";
...
//Como nos llega en la api el id de la categoria necesitamos un diccionario para interpretarlo
const arrayMovies = {
  Action: 28,
  Adventure: 12,
  Animation: 16,
...
const url_category = window.location.search
  .substring(1)
  .split("=")[1]
  .replace("+", " ")
  .trim();
...
//Para llamar a la funcion de recibir los datos de la api con la categoria que queramos
...
function showData(data) {
  ...
  data.forEach((movie) => {
    ...
    const { title, poster_path, vote_average, genre_ids, backdrop_path, id } =
      movie;
    const mainEl = document.createElement("div");

    //Creamos un contador para limitar las busquedas a 16
    if (count <= 16) {
      mainEl.innerHTML = `
      <a href="#">
